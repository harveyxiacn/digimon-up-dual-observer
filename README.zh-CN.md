...
- 装备流程必须同时确认大号“出售”和“装备”按钮：
  ...
  - 替换后的旧装备变为红色下箭头：出售；
  - 初次弹窗就是红色下箭头：直接出售；
  - 箭头不明确：按 OCR 读到的词条优先级比较（同时有暴击发生率+技能暴击发生率、只有暴击发生率、只有技能暴击发生率、均无）；仅当新装备严格更高时装备，否则出售；
  - 词条 OCR 无法可靠读取：不操作并记录警告。
...
**总开关**控制所有 ADB 点击（关闭即观察模式）。以下四项可独立切换并保存在本机：**任务监控与领取**、**装备自动处理**、**食物气泡自动点击**、**Discord 自动通知**。长期未完成的任务可以单独暂停，不影响其他功能，之后可随时恢复。只要 Webhook 有效，手动**测试**按钮始终会发送，不受自动通知开关影响。
...
Discord 完整链路会截取当前模拟器画面，尽可能附上设备标签及任务/OCR 上下文，并以附件发送。首次事件可立即通知；重复事件遵守各自冷却时间，发送失败会延后重试。遇到 Discord 限流或无效/已删除的 Webhook 时会暂时抑制重复发送。清空 Webhook 输入会删除本机保存的 secret。`.env` 在本机是明文，绝不可共享；OS 环境变量优先于 UI 与 `.env`。截图和 OCR 文字可能包含任务信息、设备标签，请将 Discord 与本地日志视为敏感资料。
...
- 没有绿色/红色箭头时，词条回退逻辑仅装备优先级严格更高的新装备；同级或更低则出售，OCR 不可靠时始终不操作。
- 装备面板无法可靠读取或动作按钮不成对时，始终不操作。
