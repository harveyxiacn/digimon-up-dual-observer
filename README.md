...
一个面向 Windows 与主流安卓模拟器的 Digimon UP 画面监控器。程序通过 ADB 直接取得安卓画面并发送点击，不依赖鼠标位置，因此模拟器可以最小化或放在不同显示器上。默认只启用一个监控通道；需要时可切换到多模拟器模式。

![像素风模拟器监控界面](docs/ui-preview.png)

## 已实现

- 默认选择并监控一台 ADB 模拟器，只显示一个实时画面，避免无意增加资源占用。
- 用户开启“多模拟器模式”后，可同时选择并显示最多两台模拟器。
...
5. 默认单模拟器模式只允许勾选一台。若需要双开，开启“多模拟器模式”后再勾选第二台；Quest、手机等其他 ADB 设备默认不会被勾选。
