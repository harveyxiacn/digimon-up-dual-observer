...
**總開關**控制所有 ADB 點擊（關閉即觀察模式）。以下四項可獨立切換並保存在本機：**任務監控與領取**、**裝備自動處理**、**食物氣泡自動點擊**、**Discord 自動通知**。長期未完成的任務可以單獨暫停，不影響其他功能，之後可隨時恢復。只要 Webhook 有效，手動**測試**按鈕始終會傳送，不受自動通知開關影響。
...
Discord 完整鏈路會擷取目前模擬器畫面，盡可能附上裝置標籤及任務/OCR 內容，並以附件傳送。首次事件可立即通知；重複事件會遵守各自的冷卻時間，傳送失敗會延後重試。遇到 Discord 限流或無效/已刪除的 Webhook 時會暫時抑制重複傳送。清空 Webhook 輸入會刪除本機儲存的 secret。`.env` 在本機是明文，絕不可共享；OS 環境變數優先於 UI 與 `.env`。截圖和 OCR 文字可能包含任務資訊、裝置標籤，請將 Discord 與本機記錄視為敏感資料。
